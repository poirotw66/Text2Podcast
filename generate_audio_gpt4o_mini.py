#!/usr/bin/env python3
"""
Generate audio files using OpenAI TTS API with GPT-4o-mini-tts model
Generate audio for two-person dialogue from transcript files with emotion markers
Uses GPT-4o-mini-tts model for potentially better emotion expression
"""
...
SPEAKER_VOICES = {
    "Speaker 1": "onyx",      # More professional and clear voice
    "Speaker 2": "marin"    # More lively and energetic voice
}

# TTS model configuration - Using GPT-4o-mini-tts
# Note: If this model doesn't exist, the API will return an error
# You can fallback to "tts-1" or "tts-1-hd" if needed
TTS_MODEL = "gpt-4o-mini-tts"  # GPT-4o-mini TTS model
AUDIO_FORMAT = "mp3"    # Output format: mp3, opus, aac, flac

# Emotion to natural text enhancement mapping for GPT-4o-mini-tts
# GPT-4o-mini-tts has better understanding of emotions and context
# Use subtle, natural enhancements that work with the model's intelligence
EMOTION_ENHANCEMENTS = {
    "excited": {
        "prefix": "",  # GPT-4o-mini-tts can infer excitement from context and punctuation
        "suffix": "！",
        "punctuation_replace": True  # Exclamation mark is sufficient
    },
    "calm": {
        "prefix": "",
        "suffix": "。",
        "punctuation_replace": False  # Natural period for calm delivery
    },
    "curious": {
        "prefix": "",  # Question mark naturally conveys curiosity
        "suffix": "？",
    ...
    "friendly": {
        "prefix": "",
        "suffix": "。",
        "punctuation_replace": False  # Friendly tone from natural speech
    },
    "serious": {
        "prefix": "",
        "suffix": "。",
        "punctuation_replace": False  # Serious tone from content and delivery
    },
    "playful": {
        "prefix": "",  # Let GPT-4o-mini-tts interpret playfulness from context
        "suffix": "。",
        "punctuation_replace": False
    },
    "thoughtful": {
        "prefix": "",
        "suffix": "……",  # Ellipsis suggests thoughtful pause and reflection
        "punctuation_replace": True
    },
    "surprised": {
        "prefix": "",  # Exclamation mark conveys surprise effectively
        "suffix": "！",
        "punctuation_replace": True
    },
    "intense": {
        "prefix": "",
        "suffix": "——",  # Dash suggests intensity and emphasis
        "punctuation_replace": True
    },
    "reflective": {
        "prefix": "",
        "suffix": "……",  # Ellipsis for reflective pause
        "punctuation_replace": True
    },
    "concerned": {
        "prefix": "",
        "suffix": "……",  # Ellipsis suggests concern and hesitation
        "punctuation_replace": False
    },
    "sad": {
        "prefix": "",
        "suffix": "……",  # Ellipsis suggests sadness and emotional weight
        "punctuation_replace": False
    },
    "happy": {
        "prefix": "",
        "suffix": "！",
        "punctuation_replace": True  # Exclamation conveys happiness
    },
    "angry": {
        "prefix": "",
        "suffix": "！",
        "punctuation_replace": True  # Exclamation with context conveys anger
    },
    "whisper": {
        "prefix": "",
        "suffix": "。",
        "punctuation_replace": False  # Whisper tone from context
    },
    "loud": {
        "prefix": "",
        "suffix": "！",
        "punctuation_replace": True  # Exclamation suggests loud delivery
    },
}
...
def enhance_text_with_emotion(text: str, emotion: str = None) -> str:
    """
    Enhance text with emotion for GPT-4o-mini-tts
    GPT-4o-mini-tts has better emotion understanding, so we use subtle punctuation
    and rely on the model's intelligence to interpret emotions from context
...
def generate_single_audio(
    ...
    """
    Generate a single audio file with emotion support using GPT-4o-mini-tts
    ...
    try:
        ...
        # Try to use GPT-4o-mini-tts, fallback to tts-1 if model doesn't exist
        try:
            ...
            # If model doesn't exist, try fallback to tts-1
            if "model" in str(model_error).lower() or "invalid" in str(model_error).lower():
                with _print_lock:
                    print(f"  ⚠ Warning: Model '{model}' not available, falling back to 'tts-1'")
                response = openai_client.audio.speech.create(
                    model="tts-1",
                    voice=voice,
                    input=enhanced_text,
...
def generate_audio_from_transcript(
    ...
    """
    Generate all audio from transcript file with emotion support using GPT-4o-mini-tts (using multithreading)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate audio using OpenAI TTS API with GPT-4o-mini-tts model and emotion support"
    )
    parser.add_argument(
        "transcript_file",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=TTS_MODEL,
        help=f"TTS model (default: {TTS_MODEL}, fallback to tts-1 if model unavailable)"
    )
    parser.add_argument(
        "--merge",
        action="store_true",
    ...
    if not args.transcript_file:
        parser.error("transcript_file is required (unless using --list-emotions)")
